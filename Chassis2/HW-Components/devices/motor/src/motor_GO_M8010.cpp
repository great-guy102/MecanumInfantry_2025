/**
 *******************************************************************************
 * @file      : motor_GO_M8010.cpp
 * @brief     : 宇树 GO-M8010 电机类
 * @history   :
 *  Version     Date            Author          Note
 *  V0.9.0      2024-07-04      Caikunzhen      1. 未测试版本
 ...
 *  宇树 A1 电机不通用），该模块具有两路 RS485 通信接口，一路上上位机配置电机 ID 0~2
 *  对应电机 ID 1~3，另一路对应电机 ID 4~6
...
static const MotorBaseInfo kGO_M8010MotorBaseInfo{
    .raw_input_lim = 0x7FFF,
    .torq_input_lim = 23.7f,
    .curr_input_lim = 40.0f,
    .torq_const = 0.63895f / 6.33,
    .redu_rat = 6.33f,
    .angle_rat = 2 * PI / 32768,
    .vel_rat = 2 * PI / 256,
    .curr_rat = kInvalidValue,
    .torq_rat = 1.0f / 256,
    .cross_0_value = 32768u,
...
GO_M8010::GO_M8010(uint8_t id, const OptionalParams& opt)
...
#pragma endregion
  ...
  motor_info_ = kGO_M8010MotorBaseInfo;

  /* 根据 ID 设定报文 ID */
  motor_info_.tx_id = id <= 3 ? kTx1_3_ : kTx4_6_;
...
GO_M8010& GO_M8010::operator=(const GO_M8010& other)
{
  if (this != &other) {
...
GO_M8010::GO_M8010(GO_M8010&& other) : Motor(std::move(other))
{
  temp_ = other.temp_;
}

GO_M8010& GO_M8010::operator=(GO_M8010&& other)
{
  if (this != &other) {
...
bool GO_M8010::decode(size_t len, const uint8_t* data)
{
  /* 变量检查 */
#pragma region
...
bool GO_M8010::encode(size_t& len, uint8_t* data)
{
  /* 变量检查 */
#pragma region
  HW_ASSERT(data, "data is nullptr");
...
#pragma endregion
  ...
  uint8_t index = (motor_info_.id - 1) % 3;
...
void GO_M8010::init(uint8_t id, const OptionalParams& opt)
{
  /* 变量检查 */
#pragma region
  HW_ASSERT(1 <= id && id <= 6, "Error id: %d", id);
