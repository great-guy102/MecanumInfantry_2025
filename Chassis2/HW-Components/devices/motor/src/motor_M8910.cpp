...
static const MotorBaseInfo kM8910MotorBaseInfo{
    .raw_input_lim = 0x7FFF,
    .torq_input_lim = 5.0f,
    .curr_input_lim = 75.0f,
    .torq_const = 0.07f,
    .redu_rat = 1.0f,
    .angle_rat = 2 * PI / 0xFFFF,
    .vel_rat = 400.0f / 0x8000,
    .curr_rat = 100.0f / 32768,
...
M8910::M8910(uint8_t id, const OptionalParams& opt)
...
#pragma endregion
  ...
  motor_info_ = kM8910MotorBaseInfo;
...
M8910& M8910::operator=(const M8910& other)
{
  if (this != &other) {
...
M8910::M8910(M8910&& other) : Motor(std::move(other))
{
  power_ = other.power_;
}

M8910& M8910::operator=(M8910&& other)
{
  if (this != &other) {
...
bool M8910::decode(size_t len, const uint8_t* data)
{
  /* 变量检查 */
#pragma region
...
#pragma endregion
  ...
  torq_ = raw2Torq(raw);
  curr_ = raw2Curr(raw);
  power_ = (static_cast<int16_t>((data[6] << 8) | data[7])) *
           1000.0f / 0x8000;
...
bool M8910::encode(size_t& len, uint8_t* data)
{
  /* 变量检查 */
#pragma region
  HW_ASSERT(data, "data is nullptr");
...
void M8910::init(uint8_t id, const OptionalParams& opt)
{
  /* 变量检查 */
#pragma region
  HW_ASSERT(1 <= id && id <= 8, "Error id: %d", id);
