/**
 *******************************************************************************
 * @file      : steered_standard_wheel.hpp
 * @brief     :
 * @history   :
...
#ifndef HW_COMPONENTS_ALGORITHMS_IK_SOLVER_CHASSIS_WHEEL_STEERED_STANDARD_WHEEL_HPP_
#define HW_COMPONENTS_ALGORITHMS_IK_SOLVER_CHASSIS_WHEEL_STEERED_STANDARD_WHEEL_HPP_
...
class SteeredStandardWheel : public Wheel
{
 public:
  enum OptMask {
    ...
    /*/< 当车轮转速为 0 时，保持上一次的速度矢量角度 */
    kOptMaskKeepLastThetaVelRefWhen0 = 1u << 3,
  };

  explicit SteeredStandardWheel(const WheelParams& params) : Wheel(params) {}
  SteeredStandardWheel(const SteeredStandardWheel&) = default;
  SteeredStandardWheel& operator=(const SteeredStandardWheel&) = default;
  SteeredStandardWheel(SteeredStandardWheel&&) = default;
  SteeredStandardWheel& operator=(SteeredStandardWheel&&) = default;

  virtual ~SteeredStandardWheel(void) = default;
      ...
      const float* theta_vel_fdb_ptr = nullptr) override
  {
    ...
    // 无侧滑约束求解
    float beta_ref = 0;
    if (IsBitsSet(kOptMaskKeepLastThetaVelRefWhen0, params_.opt_mask)) {
    ...
    if (IsBitsSet(kOptMaskMinThetaVelDelta, params_.opt_mask)) {
      theta_vel_delta = hello_world::AngleNormRad(beta_ref - beta_fdb);
...
#endif /* HW_COMPONENTS_ALGORITHMS_IK_SOLVER_CHASSIS_WHEEL_STEERED_STANDARD_WHEEL_HPP_ */
