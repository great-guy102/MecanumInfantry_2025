...
M2006::M2006(uint8_t id, const OptionalParams& opt)
...
M2006& M2006::operator=(const M2006& other)
{
  if (this != &other) {
...
M2006::M2006(M2006&& other) : Motor(std::move(other)) {}

M2006& M2006::operator=(M2006&& other)
{
  if (this != &other) {
...
bool M2006::decode(size_t len, const uint8_t* data)
{
  /* 变量检查 */
#pragma region
...
bool M2006::encode(size_t& len, uint8_t* data)
{
  /* 变量检查 */
#pragma region
  HW_ASSERT(data, "data is nullptr");
...
void M2006::init(uint8_t id, const OptionalParams& opt)
{
  /* 变量检查 */
#pragma region
  HW_ASSERT(1 <= id && id <= 8, "Error id: %d", id);
