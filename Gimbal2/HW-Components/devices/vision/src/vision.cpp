...
void Vision::decodeRxData(size_t len, const uint8_t *data)
{
  ...
  temp_int16 = static_cast<int16_t>((data[3] << 8) | data[4]);
  rx_data_.pitch = Deg2Rad(temp_int16 / 100.0f);

  temp_int16 = static_cast<int16_t>((data[5] << 8) | data[6]);
  ...
  rx_data_.vtm_x = static_cast<uint16_t>(data[8] << 8 | data[9]);
  rx_data_.vtm_y = static_cast<uint16_t>(data[10] << 8 | data[11]);
};
